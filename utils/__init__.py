...
_real_pkg_path = 'src.backend.utils'
...
try:
    ...
    for _name, _mod in list(_sys.modules.items()):
        if _name.startswith(f"{_real_pkg_path}."):
            _alias_name = _name.replace(_real_pkg_path, 'utils', 1)
            _sys.modules[_alias_name] = _mod

    # Finally, register this module as an alias of the real one for future imports
    _sys.modules['utils'] = _real_pkg

    # Attempt to make submodules directly importable (e.g., from utils import distributions)
    ...
    print(f"Warning: Real package '{_real_pkg_path}' not found for alias 'utils'.")
