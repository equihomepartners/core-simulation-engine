"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var websocketClient_1 = require("./websocketClient");
// Test script for WebSocketClient
var testWebSocket = function () { return __awaiter(void 0, void 0, void 0, function () {
    var token, client, unsubscribe, response, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                console.log('Starting WebSocket test...');
                token = localStorage.getItem('auth_token') || 'test_token';
                client = new websocketClient_1.WebSocketClient(token, {
                    baseUrl: 'ws://localhost:5005/ws',
                    reconnectInterval: 1000,
                    maxReconnectAttempts: 3,
                    pingInterval: 30000,
                    connectionStatusCallback: function (status, details) {
                        console.log("Connection status update: ".concat(status), details || '');
                    }
                });
                _a.label = 1;
            case 1:
                _a.trys.push([1, 10, , 11]);
                console.log('Attempting to connect to WebSocket server...');
                return [4 /*yield*/, client.connect()];
            case 2:
                _a.sent();
                console.log('WebSocket connected successfully.');
                // Test subscription
                console.log('Subscribing to test channel...');
                return [4 /*yield*/, client.subscribe('test_channel', 'test_resource', function (data) {
                        console.log('Received data from subscription:', data);
                    })];
            case 3:
                unsubscribe = _a.sent();
                console.log('Successfully subscribed to test_channel:test_resource');
                // Test sending a message
                console.log('Sending test message...');
                return [4 /*yield*/, client.send({ event: 'test_event', data: { message: 'Hello, WebSocket!' } })];
            case 4:
                response = _a.sent();
                console.log('Response from test message:', response);
                // Test reconnection logic by simulating a disconnect (if possible)
                console.log('Testing reconnection logic by closing connection...');
                client.disconnect();
                console.log('Disconnected. Waiting to see reconnection attempts...');
                // Wait for reconnection attempts
                return [4 /*yield*/, new Promise(function (resolve) { return setTimeout(resolve, 5000); })];
            case 5:
                // Wait for reconnection attempts
                _a.sent();
                if (!!client['isConnected']) return [3 /*break*/, 7];
                console.log('Reconnection in progress or failed. Attempting manual reconnect...');
                return [4 /*yield*/, client.connect()];
            case 6:
                _a.sent();
                console.log('Manual reconnection attempted.');
                return [3 /*break*/, 8];
            case 7:
                console.log('Reconnection successful.');
                _a.label = 8;
            case 8:
                // Keep the connection open for a while to observe behavior
                console.log('Keeping connection open for 1 minute to observe behavior...');
                return [4 /*yield*/, new Promise(function (resolve) { return setTimeout(resolve, 60000); })];
            case 9:
                _a.sent();
                unsubscribe();
                client.disconnect();
                console.log('WebSocket test completed successfully.');
                return [3 /*break*/, 11];
            case 10:
                error_1 = _a.sent();
                console.error('WebSocket test failed:', error_1);
                return [3 /*break*/, 11];
            case 11: return [2 /*return*/];
        }
    });
}); };
// Run the test
console.log('Initiating WebSocket test suite...');
testWebSocket().then(function () { return console.log('WebSocket test suite finished.'); });
