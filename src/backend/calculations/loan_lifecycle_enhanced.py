...
def generate_reinvestment_loans_enhanced(
    ...
    # Adjust loan size to match reinvestment amount
    adjusted_loan_size = reinvestment_amount / Decimal(num_loans)

    # Generate zone allocation based on fund's target allocation
    zone_allocation = fund.zone_allocations

    # Allocate loans to zones
    zone_loan_counts = {}
    remaining_loans = num_loans

    for zone in sorted(zone_allocation.keys(), key=lambda z: zone_allocation[z], reverse=True):
        # Calculate desired loan count for this zone
        desired_count = int(zone_allocation[zone] * Decimal(num_loans))

        # Ensure we don't exceed remaining loans
        zone_loan_counts[zone] = min(desired_count, remaining_loans)
        remaining_loans -= zone_loan_counts[zone]

    # Allocate any remaining loans to the zone with highest allocation
    if remaining_loans > 0:
        highest_zone = max(zone_allocation.items(), key=lambda x: x[1])[0]
        zone_loan_counts[highest_zone] += remaining_loans
    ...
    for zone, loan_count in zone_loan_counts.items():
        ...
        # Calculate zone-specific loan amount
        zone_loan_amount = reinvestment_amount * zone_allocation[zone] / Decimal(loan_count)
